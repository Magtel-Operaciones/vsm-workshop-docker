...
{#snippet improvementBadge(value, higherIsBetter = true)}
  ...
  {:else}
    ...
    <span
      class="inline-flex items-center gap-1 font-medium {isPositive
        ? 'text-emerald-600'
        : 'text-red-600'}"
    >
      {#if isPositive}
...
{#if scenarios.length > 0 || comparisonResults}
  <div class="bg-white border-t border-slate-200 p-4">
    <h3 class="text-sm font-semibold text-slate-900 mb-3">What-If Scenarios</h3>
    ...
    {#if scenarios.length > 0}
      <div class="space-y-2 mb-4">
        {#each scenarios as scenario (scenario.id)}
          <div
            class="flex items-center justify-between p-3 rounded-lg border transition-colors {activeScenarioId ===
            scenario.id
        ...
        <table class="w-full text-sm">
          <thead class="bg-slate-100">
            <tr>
              <th
                scope="col"
                class="px-4 py-2 text-left font-medium text-slate-700"
              >
                Metric
              </th>
              <th
                scope="col"
                class="px-4 py-2 text-right font-medium text-slate-700"
              >
                Baseline
              </th>
              <th
          ...
          <tbody class="divide-y divide-slate-200">
            <tr>
              ...
              <td class="px-4 py-2 text-right text-slate-700">
                {formatDuration(
                  comparisonResults.baseline.avgLeadTime,
                  minutesPerWorkDay
                )}
              </td>
              <td class="px-4 py-2 text-right text-slate-700">
                {formatDuration(
                  comparisonResults.scenario.avgLeadTime,
                  minutesPerWorkDay
                )}
              </td>
              <td class="px-4 py-2 text-right">
                {@render improvementBadge(
                  comparisonResults.improvements.leadTime,
                  true
                )}
              </td>
            </tr>
            <tr>
              ...
              <td class="px-4 py-2 text-right">
                {@render improvementBadge(
                  comparisonResults.improvements.throughput,
                  true
                )}
              </td>
            </tr>
